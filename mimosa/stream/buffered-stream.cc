  ...
  namespace stream
  {
    ...
    BufferedStream::write(const char * data, uint64_t nbytes, runtime::Time timeout)
    {
      ...
      while (nbytes + readyWrite() >= buffer_size_)
      {
        ...
        for (auto it = wbuffers_.cbegin(); it != wbuffers_.cend(); ++it, ++p)
        {
          p->iov_base = it->data();
        ...
        iov[iov_cnt - 1].iov_base = data;
        iov[iov_cnt - 1].iov_len  = nbytes;

        // write and remove completed buffers
        int64_t wbytes = stream->writev(iov, iov_cnt, timeout);
        if (wbytes < 0)
        {
    ...
    BufferedStream::read(char * data, uint64_t nbytes, runtime::Time timeout)
    {
      if (rappend_ == rpos_)
      {
        ...
        rpos_    = 0;
        rappend_ = 0;
        int64_t rbytes = stream_->read(rbuffer->data(), rbuffer->size(), timeout);
        if (rbytes <= 0)
          return 0;
        rappend_ = rbytes;
      }
